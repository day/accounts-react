    ...
    if (this.config.logUserMetaData) {
      ...
      if (Meteor.isServer) {
        Meteor.onConnection(function (connection) {
          this.clientAddress = connection.clientAddress
          this.clientLocationData = {message: "Configure Meteor.settings.ipStackAccessKey to log IP location data."}
          if (Meteor.settings.ipStackAccessKey) {
            this.locationData = HTTP.get("http://api.ipstack.com/" + this.clientAddress + "?access_key=" + Meteor.settings.ipStackAccessKey)
          }
        });
        Meteor.settings.public.userMetaData.ipAddress = this.clientAddress
        Meteor.settings.public.userMetaData.locationData = this.clientLocationData
      }
      if (Meteor.isClient) {
