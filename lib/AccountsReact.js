    ...
    if (this.config.logUserMetaData) {
      ...
      if (Meteor.isServer) {
        Meteor.onConnection(function(connection){
          Meteor.settings.public.ipAddress = connection.clientAddress
        });
        Meteor.methods({
          getLocationData: function () {
            ...
            try {
              if (Meteor.settings.ipStackAccessKey) {
                var result = HTTP.get("http://api.ipstack.com/" + Meteor.settings.public.ipAddress + "?access_key=" + Meteor.settings.ipStackAccessKey)
              }
              else {
                return {message: "Configure Meteor.settings.ipStackAccessKey to log IP location data."}
              }
            } catch (error) {
      ...
      if (Meteor.isClient) {
        var self = this;
        self.locationData = new ReactiveVar(null);
        Meteor.call("getLocationData", function(error, result) {
          if (!error) {
            self.locationData.set(result);
          } else {
            console.log(error);
          }
        });
        Session.set('userMetaData', {
          locationData: self.locationData.get(),
