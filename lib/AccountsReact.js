    ...
    if (this.config.logUserMetaData) {
        ...
        Meteor.methods({
          getLocationData: function () {
            if (Meteor.settings.ipStackAccessKey) {
              return HTTP.get("http://api.ipstack.com/" + this.connection.clientAddress + "?access_key=" + Meteor.settings.ipStackAccessKey)
            }
            else {
      ...
      if (Meteor.isClient) {
        ...
        Meteor.call("getLocationData", function(error, result) {
          if (!error) {
            Meteor.settings.public.userMetaData.locationData = result;
          }
        });
      }
    }
