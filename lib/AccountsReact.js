...
class AccountsReact_ {
  ...
  setUserMetaData () {
    if (this.config.logUserMetaData) {
      if (!Meteor.settings.public.userMetaData) {
        Meteor.settings.public.userMetaData = {}
      }
      if (Meteor.isServer) {
        Meteor.onConnection(function(connection){
          Meteor.settings.public.userMetaData.ipAddress = connection.clientAddress
          Meteor.settings.public.userMetaData.locationData = {message: "Configure Meteor.settings.ipStackAccessKey to log IP location data."}
          console.log("======================================================")
          console.log("******************************************************")
          console.log("ipStack Access Key: " + Meteor.settings.ipStackAccessKey)
          if (Meteor.settings.ipStackAccessKey) {
            Meteor.settings.public.userMetaData.locationData = HTTP.get("http://api.ipstack.com/" + connection.clientAddress + "?access_key=" + Meteor.settings.ipStackAccessKey)
          }
          console.log("******************************************************")
          console.log(Meteor.settings.public.userMetaData)
          console.log("******************************************************")
        })
      }
      if (Meteor.isClient) {
        Meteor.settings.public.userMetaData.deviceInfo = navigator.userAgent
        Meteor.settings.public.userMetaData.displaySize = screen.width + 'x' + screen.height
      }
    }
  }
...
Meteor.startup(() => {
  ...
  const prefix = 'day:accounts-react'
  const components =
    Package[prefix + '-semantic-ui']
    // ...
